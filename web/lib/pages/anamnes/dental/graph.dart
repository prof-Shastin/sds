...
Future<Uint8List> graphDental(Model model, int width, int height) async {
  final post = {'patient_id': model.patient.value.id, 'anamnes_id': 2};
  final d = await model.post('/anamnes/list', post);
  final map = <int, bool>{};
  for (final it in d) map[it['item_id'] as int] = it['check'] as bool;
  final v = data.where((e) => e[1] == 0).map((e) {
    final l = data.where((c) => c[1] == e[0]).toList();
    return l.where((c) => map[c[0]] == true).length / l.length;
  }).toList();
